...
function openAML(c){
  ...
  document.getElementById('aml-profile').innerHTML=[['Occupation',pf.occupation],['Country',pf.country],['PEP Status','<span title="'+pepTip+'" style="color:'+(pepHigh?'#ff4d6a':'#00e68a')+';font-weight:700;cursor:help;border-bottom:1px dotted currentColor">'+pf.pep+'</span>'],['Risk Score','<span title="'+rsTip+'" style="color:'+(rsHigh?'#ff4d6a':rsMed?'#ffb833':'#00e68a')+';font-weight:700;cursor:help;border-bottom:1px dotted currentColor">'+pf.riskScore+'/100</span>'],['Account Age',pf.accountAge],['Declared Income','<span title="'+incTip+'" style="cursor:help;border-bottom:1px dotted #4a6a8a">'+pf.income+'</span>']].map(function(r){return '<div style="color:#4a6a8a;font-weight:600">'+r[0]+'</div><div>'+r[1]+'</div>';}).join('');
  document.getElementById('aml-txs').innerHTML=c.transactions.map(function(tx){return '<div class="gc-s p-3 tx-card flex justify-between items-center gap-3" style="'+(tx.flag?'border-left:3px solid #ff4d6a':'')+'"><div class="flex-1 min-w-0"><div style="font-family:\'Anybody\',sans-serif;font-size:.88rem;font-weight:700;color:'+(tx.flag?'#ff4d6a':'#fff')+'">'+tx.amount+'</div><div style="font-size:.62rem;color:#4a6a8a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+tx.to+'</div></div><div class="text-right flex-shrink-0"><div style="font-size:.58rem;color:#4a6a8a;font-family:\'JetBrains Mono\',monospace">'+tx.date+'</div><div style="font-size:.56rem;color:#8ba4c0">'+tx.country+'</div></div></div>';}).join('');
  renderNetworkAML(c.network,c.transactions);renderCategorizedFlags('aml-flags',c.flags);document.getElementById('aml-notes').value=state.notes[c.id]||'';renderFlagSummary('aml-flagsum',c.flags);
  var txFlagged=c.transactions.filter(function(t){return t.flag;}).length;setCollCount('aml-txs-count',c.transactions.length,txFlagged);showScreen('aml');
}
...
function renderFlagSummary(elId,flags){var html='<div class="fsum"><div class="fsum-title">🚩 Flag Summary</div>';FLAG_CATS.forEach(function(cat){var count=0;flags.forEach(function(f){var fl=f.toLowerCase();for(var i=0;i<cat.keys.length;i++){if(fl.indexOf(cat.keys[i])!==-1){count++;break;}}});html+='<div class="fsum-row"><span class="fsum-label">'+cat.icon+' '+cat.label+'</span><span class="fsum-val '+(count>0?'hit':'ok')+'">'+count+'</span></div>';});html+='<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,77,106,.08)"><div class="fsum-row"><span class="fsum-label" style="font-weight:700;color:#fff">Total Flags</span><span class="fsum-val hit" style="font-size:.62rem">'+flags.length+'</span></div></div></div>';var el=document.getElementById(elId);if(el){el.innerHTML=flags.length?html:'';}}
